...
void ArmPlugin::Load(physics::ModelPtr _parent, sdf::ElementPtr /*_sdf*/)
{
	...
	// TODO - Create our node for camera communication
 
	// TODO - Subscribe to camera topic - Done
 
	// TODO - Create our node for collision detection
	collisionNode->Init();
		...
		*/
		...
		else
		{
			...
			if( episodeFrames > 1 )
			{
				...
				//const float distThresh = 1.0f;		// maximum distance to the goal
				...
				rewardHistory = avgGoalDelta * distGoal;	// exp(-GAMMA_FALLOFF * distGoal) * 0.1f;
				newReward     = true;
			}
